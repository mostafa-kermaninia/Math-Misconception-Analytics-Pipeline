...
def preprocess_data(df_questions, df_answers, df_misconceptions):
    ...
    for _, row in df_questions.iterrows():
        ...
        construct_subject = row["ConstructSubject"]
        subject = row["SubjectName"]
        correct_text = df_answers.loc[
            df_answers["QuestionId"] == row["QuestionId"]
            and df_answers["AnswerType"] == row["CorrectAnswer"],
            "AnswerText"
            ]
        ...
        options = {
            'A': df_answers.loc[
            df_answers["QuestionId"] == row["QuestionId"]
            and df_answers["AnswerType"] == "A",
            "AnswerText"
            ],
            'B': df_answers.loc[
            df_answers["QuestionId"] == row["QuestionId"]
            and df_answers["AnswerType"] == "B",
            "AnswerText"
            ],
            'C': df_answers.loc[
            df_answers["QuestionId"] == row["QuestionId"]
            and df_answers["AnswerType"] == "C",
            "AnswerText"
            ],
            'D': df_answers.loc[
            df_answers["QuestionId"] == row["QuestionId"]
            and df_answers["AnswerType"] == "D",
            "AnswerText"
            ]
        }
...
if __name__ == "__main__":
    ...
    # Preprocess data
    preprocessed_data = preprocess_data(df_questions, df_answers, df_misconceptions)
